...
# flips the bits for the bit_array ar corresponding indices.
def flip(bit_array, indices):
...
if __name__ == '__main__':
    ...
    textlist = generate_strings(text, 1, 5)
    boxplot_data = []
    for i in range(16):
        boxplot_data.append([])
    for str2 in textlist:
        temp_r, temp_rounds = d.encrypt(key, str2)
        for i in range(16):
            temp_hm = find_hm(rounds[i], temp_rounds[i])
            boxplot_data[i].append(temp_hm)
    print("(i)5 Different Plain Texts")
    print("Hamming Distance Round Wise is:", boxplot_data)
    fig = plt.figure("(i)5 Different Plaintexts")
    ...
    for i in range(1, 5+1):
        list1 = generate_strings(text, i, 1)
        textlist.append(list1[0])
    boxplot_data = []
    for i in range(16):
        boxplot_data.append([])
    for str1 in textlist:
        temp_r, temp_rounds = d.encrypt(key, str1)
        for i in range(16):
            temp_hm = find_hm(rounds[i], temp_rounds[i])
            boxplot_data[i].append(temp_hm)
    print("(ii)5 Different Hamming Distances")
    print("Hamming Distance Round Wise is ", boxplot_data)
    fig = plt.figure("(ii) 5 Different Hamming Distances")
    plt.xlabel("Number of Rounds")
    plt.ylabel("Hamming Dist")
    plt.boxplot(boxplot_data)
    plt.show()

    # To generate 5 secret keys with Hamming distance of 1 form base string.
    gen_key_list = generate_strings(key, 1, 5, flg=True)
    boxplot_data = []
    for i in range(16):
        boxplot_data.append([])
    for temp_key in gen_key_list:
        temp_r, temp_rounds = d.encrypt(temp_key, text)
        for i in range(16):
            temp_hm = find_hm(rounds[i], temp_rounds[i])
            boxplot_data[i].append(temp_hm)
    print("(iii)5 Different Secret Texts")
    print("Hamming Distance Round Wise is ", boxplot_data)
    fig = plt.figure("(iii) 5 Different Secret Keys")
    plt.xlabel("Number of Rounds")
    plt.ylabel("Hamming Dist")
    plt.boxplot(boxplot_data)
    plt.show()